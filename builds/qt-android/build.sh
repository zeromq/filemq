...
##
# Build filemq from local source

(android_build_verify_so "libfilemq.so" "libczmq.so" &> /dev/null) || {
    rm -rf "${cache}/filemq"
    (cp -r ../.. "${cache}/filemq" && cd "${cache}/filemq" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/filemq" && ./autogen.sh \
...
android_build_verify_so "libfilemq.so" "libczmq.so"
